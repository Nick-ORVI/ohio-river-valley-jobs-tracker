...
# ==============================================================================
# BLS State Employment Data Fetcher — Year-to-Date change, OH, PA, KY, WV
# Pulls Current Employment Statistics (CES), State & Area, seasonally adjusted
#
# Computes cumulative change from December of the prior year (the standard
# "since December" YTD baseline) through the target month, for each state
# and industry sector.
#
# USAGE:
#   1. Update target_year / target_month below to change the target month
#   2. Run: Rscript fetch_state_sector_ytd_data.R
#   3. Then run: Rscript state_sector_ytd_chart.R
#
...
# Same supersector codes used in fetch_state_sector_data.R
...
cat("=== BLS State Sector Year-to-Date Data Fetcher ===\n")
cat(sprintf("States       : %s\n", paste(states$state, collapse = ", ")))
cat(sprintf("Baseline     : %s %d (December, prior year)\n", "December", baseline_year))
...
output$change <- output$target_value - output$baseline_value

# ---- Display -------------------------------------------------------------------

cat(sprintf("\n%-15s %-30s %10s %10s %10s\n", "State", "Sector",
            sprintf("Dec %d", baseline_year),
...
for (i in seq_len(nrow(output))) {
  ...
  cat(sprintf("%-15s %-30s %10.1f %10.1f %+10.1f%s\n",
              ...
              output$baseline_value[i], output$target_value[i], output$change[i], marker))
}
...
for (st_name in states$state) {
  ...
  cat(sprintf("Total YTD nonfarm employment change, %-14s %+.1f (thousand)\n", st_name, total))
}

# ---- Write CSV -----------------------------------------------------------------

output_file <- "state_sector_ytd_data.csv"
write.csv(output[, c("state", "sector", "level", "change")], output_file, row.names = FALSE)
cat(sprintf("\nData written to %s\n", output_file))
cat("Now run: Rscript state_sector_ytd_chart.R\n")
